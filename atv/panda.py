...
class Panda(object):
    ...
    def signed_params(self, verb, request_uri, params={}, timestamp_str=None):
        ...
        auth_params['signature'] = generate_signature(verb, request_uri,
                                                      self.api_host,
                                                      self.secret_key,
                                                      additional_args)
        return auth_params
    ...
    def _http_request(self, verb, path, data={}):
        ...
        requests_url = '%s%s' % (self.api_url(), path + "?" +\
                                 canonical_querystring(signed_params))
...
def urlescape(s):
    s = unicode(s)
    return urllib.quote(s).replace("%7E", "~").replace(' ', '%20').replace('/',
                                                                           '%2F')
